...
export function monthlyAccounting(saleHistory = [], year, month) {
  ...
  return rows.reduce(
    (acc, sale) => {
      ...
      const vat = Number(sale.vat_cost || 0);
      ...
      acc.shippingCost += shipping;
      acc.vatCost += vat;
      acc.totalCost += purchase + repair + other + platformFee + shipping + vat;
      acc.profit += Number(
        sale.realized_profit ??
          revenue - purchase - repair - other - platformFee - shipping - vat,
      );
      return acc;
    },
    {
